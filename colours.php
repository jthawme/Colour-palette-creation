...
class ColourMachine
{
	...
	public $coloursRGB 	= array();
	public $coloursHSL 	= array();
	public $compliments = array();
	public $indexKey 	= null;
	public $main 		= null;

	function __construct($colours)
	{
		//$this->indexKey = rand(0,4);
		$this->indexKey = 5;
		$this->colours 	= $this->setKeys($colours);
		$this->main 	= $this->colours[0];

		foreach($this->colours as $k => $colour)
		{
			$this->coloursRGB[] = $this->splitHex($colour);
			$this->coloursHSL[] = $this->rgbToHsl($this->coloursRGB[$k][0],$this->coloursRGB[$k][1],$this->coloursRGB[$k][2]);
		}

		$this->dark 	= $this->determineDark();

		$this->createCompliments();
	}

	private function createCompliments()
	{
		$styles = array("normal","triadic","split","analogous","complement");
		$target = $this->coloursHSL[$this->indexKey];
		...
		foreach($styles as $k => $name)
		{
			...
			switch($name)
			{
				case "normal":
					...
					$colours[] = $this->makeHex($this->hslToRgb($h,$s,$l));
					break;
				case "triadic":
					$h = $target[0];
					$h += 120;
	...
	private function splitHex($colour)
	{
		return array_map('hexdec',str_split($colour,2));
	}

	private function makeHex($rgb)
	{
	...
	private function determineDark()
	{
		$avg = array_sum($this->coloursRGB[0]) / 3;
